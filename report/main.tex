...
\begin{titlepage} 
  ...
  \begin{minipage}{\dimexpr\linewidth-2\fboxsep-2\fboxrule} 
    ...
    \vskip1cm 
    Project report: statistical analysis pipeline for genetic data from a human population settlement model\par 
    \vskip1cm \kern0pt 
  \end{minipage}%
  }%
  \vskip0.5cm 
...
The choice of these SNPs is a result of previous studies the resulting set is defined as highly informative and allows to define the ancestry of a person based on a small quantity of markers that are highly discriminant.
...
The first pattern can be seen in the figure \ref{cox-image} and is a non-linear gradient in Asian \gls{admixture} that declines abruptly around the eastern part of Indonesia. The second one, represented in the figure \ref{lansing-image}, shows the differences of \glspl{admixture} on distinct parts of the DNA for different islands. These differences would tend to appear in a context where marriages between Asian women and Melanesian men would be favoured.
...
The simulated data from the model will be the \glspl{admixture} of the populations among the area covered by the model. The \glspl{admixture} will be measured for five parts of the DNA, the whole DNA, the mitochondrial DNA, the autosomal DNA and the X and the Y-chromosomal DNA. These \gls{admixture} values will only be for the last step of the simulation, since they will be the only values comparable to the real observed \glspl{admixture}.

\section{Agent-Based Model}
The model will an Agent-Based Model that will comprise a graph of nodes, each node corresponding to a \gls{deme}, and each edge corresponding to a possible migration route between \glspl{deme} (see figure \ref{nodesOnMap}). The agents of the model will be people living in the graph, in every node, and these agents will be able to migrate and to form families. The graph and the agents living in it will respond to a set of basic rules regulated by parameters and the goal is then to observe the emerging behaviours arising from it.
...
Numerous parameters can be set in the model. Some of them are defined once and for all and the fact that they are immutable have been done to simplify the actual use of the model by limiting the number of parameters to set. Changing them would actually not hold any meaning nor output realistic values.
...
To be able to compare two different scenarios, comparison functions have to be defined. These functions will be able to provide a value of similarity or dissimilarity when provided with observable values that characterise the scenarios. In this case, the comparison is done between the real observed data and the output of one simulation.
Every simulation output has to be treated to be of the same form than the observed data. Doing so allows them to be compared. The observed data does not include values for specific islands that are included in the simulations (see table \ref{islands}). The \gls{admixture} values for mitochondrion and Y-chromosome are also not available for every island.
