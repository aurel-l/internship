...
Given that the model will be run numerous times, with a lot of different changing values for the parameters, a robust statistical analysis framework needs to be implemented to extract meaningful information from the high quantity of generated data.
...
Every simulation output has to be treated in the same way than the observed data. Doing so allows them to be compared. The observed data does not include values for specific islands that are included in the simulations (see Table \ref{islands}). This is because nodes had to be placed in the model so that the agents could migrate, even though the empirical studies did not get samples from the corresponding areas. The \gls{admixture} values for mitochondrion and Y-chromosome are also not available for every island.
...
Two measures have been selected that can hold different information about a comparison. The mean square distance (MSD) and the partial Mantel test have been selected as they do not have a good correlation when applying them to random \gls{admixture} values, thus meaning that they do not carry the same information. They are complementary, and using them both gives a better overview of the comparison.
...
The Mantel tests have been developed to compare two matrices with the same information. The partial Mantel test also uses a third matrix, holding constant geographical distances of the cells of the matrices in order to be able to weight the values according to the actual geographical distances between the points \cite{Smo01}.

In this case, the matrices contain the values of distances in \gls{admixture} between every island in the graph and a matrix $M$ is calculated as
\begin{equation}
	M = \begin{bmatrix}
	...
	\end{bmatrix}
\end{equation}
where $d$ is the function returning the distance between the two arguments and $Ad$ is the \gls{admixture} of the $n$\textsuperscript{th} island in the graph. With the corresponding matrices for the simulation data, the real data and also the geographical distances, the partial Mantel test can be done as such
\begin{equation}
...
The stability analysis outputs different views of the data for this point in the grid. The first ones are 4 graphs, one for each type of observed DNA, for which box-plots for the \gls{admixture} values of each island are displayed (partial view in Fig. \ref {preview-boxplot}, complete view in Fig. \ref{app:stability}). The size and spread of the box-plots represent the variability of the data among the different simulations with the same input parameters. With “correct” parameters, the \gls{admixture} values are supposed to be less stable in the contact zones between the different populations than in the extremities of the graph, where the source populations live. Less stability is also expected when looking at \gls{admixture} values comprising fewer markers (mitochondrial and Y-chromosomal \gls{admixture}). And finally, lower stability is expected from islands that include fewer \glspl{deme}, and thus lower population sizes, like Alor, Aru or Bali that are composed of only one single \gls{deme} each. Any other behaviour leads to the view that the parameter set it comes from is not realistic, and thus induces abnormal model behaviour.
...
Another view that is output by the stability analysis is the graph of \gls{admixture} values as a function of the types of DNA, and for every island (Fig. \ref{app:stability-admixGradient}). This graph aims to resemble Figure \ref{lansing-image} from \cite{Lan01} as a way to compare the real data in this paper to the data generated in the simulations.
...
The whole framework encompasses the steps from the choice of the parameter sets to the inference of the posterior distributions. Figure \ref{abc} summarises the successive steps of this process with hypothetical parameters $\alpha$, $\beta$ and $\gamma$ and summary statistics $SS1$ and $SS2$. In this project, the parameters used are the ones defined in Table \ref{parameters} even though the ones set after the grid search analyses will be ignored by the ABC as they are defined once and will not change. They are simply not defined by a distribution as they are constant, and running them through the ABC is useless. The summary statistics will be the four results of the comparison functions defined in section \ref{subsection:comparisons}. These are the MSD and the partial correlation for both the X-chromosome and the autosomes.
...
The model used until is still undergoing changes, with the aim to make it more accurate and to take into account more factors. The problem of adding complexity would be to add enough complexity to keep the model relevant while still being computationally simple enough so that it can be run in an acceptable amount of time.
Several aspects can be changed, ranging from the structure of the family and how it is managed in the model, to higher level tasks like the integration of resource management at different scales of the model to try to be more accurate and have more realistic behaviours.
