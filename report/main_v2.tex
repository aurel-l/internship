...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{0.4\textwidth}
		\centering
		\includegraphics[scale=0.34]{../data/network:local.png}
		\caption{local}
		\label{network:local}
	\end{subfigure}
	~
	\begin{subfigure}[t]{0.4\textwidth}
		\centering
		\includegraphics[scale=0.34]{../data/network:office.png}
	...
	\begin{subfigure}[t]{0.4\textwidth}
		\centering
		\includegraphics[scale=0.78]{../data/network:azure.png}
	...
	\begin{subfigure}[t]{0.4\textwidth}
		\centering
		\includegraphics[scale=0.78]{../data/network:nesi.png}
		\caption{NeSI}
		\label{network:nesi}
	\end{subfigure}
...
Doing so, it would also be possible to use results from different batches and to analyse them together, thus avoiding having to redo simulations for parameter values already tested and reducing the computational resources used.
...
Whereas a standard Bayesian analysis can be used to infer a single best value for every parameter, here the use of ABC gives a \gls{posterior} distribution of parameters corresponding to the best values.
...
It is important to have a low tolerance taking into account that most of the parameter space (the borders of the space) is expected to give “bad” results.
...
The centre of the box-plot represents the median, the interquartile range is represented by the box and outside of the whiskers are possible outliers from the data.
...
The visualisations were made and adapted as the project evolved so that they could be used to help make decisions and choices.
...
The stream approach also allows the different steps of an analysis to be run simultaneously, by piping each step into the next one, effectively making the whole process runnable in parallel.
...
Secondly, a way has been found to generate the random seeds for the ABC analysis beforehand using \textsc{unix}’s random source, \texttt{/dev/urandom/}, that can generate pseudo-random values which can be used for cryptographic purposes, meaning that it is good enough in this case to avoid collisions.
...
Then, comparisons to the real data are performed, for both the autosomal and X-chromosomal DNA data (see section \ref{subsection:comparisons}) and the resulting values are displayed as seen in Figure \ref{app:sensit-comp-1d} and \ref{app:sensit-comp-2d} so they can be analysed by the analyst.
...
The parameter space for the migration probability has thus been reduced to $\left[0.1, 0.8\right]$ when doing further analyses.
...
Indeed, if the distribution shown does not correspond to the one wanted, it can mean that there was a problem somewhere in the pipeline.
...
The model used is still undergoing changes, with the aim to make it more accurate and to take into account more factors.
